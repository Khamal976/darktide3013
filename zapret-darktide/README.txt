...
winws.exe перед стартом, поэтому переключаться между стратегиями можно просто запуская
следующий файл. Нормальное поведение: чёрное окно мелькает и закрывается, а в панели задач
остаётся свёрнутое окно «zapret: darktide (...)».
Если окно пропадает без следа, проверьте процесс:
    tasklist | findstr winws
Пусто — winws не запустился: запустите файл от имени администратора, а если не помогло,
перезагрузите ПК. Строка winws.exe — всё работает.
