@echo off
chcp 65001 > nul
:: Darktide 3013: zapret-discord-youtube 1.10.x + правила для Warhammer 40,000: Darktide
:: Положите этот файл в папку zapret рядом с general.bat, а папку lists\ объедините с lists\ zapret.
:: Запуск от имени администратора. Окно не закрывать, пока играете.
...
start "zapret: %~n0" /min "%BIN%winws.exe" --wf-tcp=80,443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=443,19294-19344,50000-50100,35000-36000,%GameFilterUDP% ^
--filter-tcp=80,443 --hostlist="%LISTS%list-darktide.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=568 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_4pda_to.bin" --new ^
--filter-udp=35000-36000 --ipset="%LISTS%ipset-darktide.txt" --dpi-desync=fake --dpi-desync-repeats=12 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="%BIN%ACTIVE_GAME_UDP.bin" --dpi-desync-cutoff=n2 --new ^
